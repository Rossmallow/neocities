...
let notify_span = document.getElementById('notifier');
...
button.onclick = function() {
  ...
  if (name == '' || message == '') {
    ...
    notify_span.textContent = 'Please fill the required fields.';
    notify_span.style.color = 'red';
    return;
  }
  else {
  ...
  request.onreadystatechange = function() {
    if (request.readyState == XMLHttpRequest.DONE) {
        if (request.status == 204) {
          ...
          notify_span.textContent = 'Submission recieved. Thanks for your message!';
          name_input.value = '';
          contact_input.value = '';
          msg_input.value = '';
          countCharacters()
